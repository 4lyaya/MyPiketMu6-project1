...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Absensi</h1>
            ...
            <form action="{{ route('guru.absences.update', $absence) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="note" name="note" rows="3" placeholder="Tambahkan catatan tambahan (opsional)"
                        class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('note') border-red-500 @enderror">{{ old('note', $absence->note) }}</textarea>
                    @error('note')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
