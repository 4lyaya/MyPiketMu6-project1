    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Perbarui data absensi guru</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <form action="{{ route('guru.absences.update', $absence) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <!-- Tanggal -->
                    <div>
                        <label for="replaced_at" class="block mb-2 text-sm font-medium text-gray-700">
                    ...
                    <!-- Guru Absen -->
                    <div>
                        <label for="absent_teacher_id" class="block mb-2 text-sm font-medium text-gray-700">
                    ...
                    <!-- Kelas -->
                    <div>
                        <label for="classroom_id" class="block mb-2 text-sm font-medium text-gray-700">
                ...
                <!-- Jam Pelajaran -->
                <div>
                    <label class="block mb-3 text-sm font-medium text-gray-700">Jam Pelajaran <span
                            class="text-red-500">*</span></label>
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                        @php $selectedPeriods = old('periods', $absence->getSelectedPeriods()); @endphp
                        @for ($i = 1; $i <= 10; $i++)
                            <label
                                class="flex items-center gap-2 p-3 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer transition">
                                <input type="checkbox" name="periods[]" value="{{ $i }}"
                                    {{ in_array($i, $selectedPeriods) ? 'checked' : '' }}
                                    class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500">
                                <span class="text-sm font-medium text-gray-700">Jam {{ $i }}</span>
                            </label>
                        @endfor
                    </div>
                    @error('periods')
                ...
                <div>
                    ...
                    <textarea id="note" name="note" rows="3" placeholder="Tambahkan catatan tambahan (opsional)"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('note') border-red-500 @enderror">{{ old('note', $absence->note) }}</textarea>
                    @error('note')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
                    <button type="submit"
                        class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition">
                        <i class="fas fa-save"></i>
                        Update Absensi
                    </button>
                    <a href="{{ route('guru.absences.index') }}"
                        class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200 transition">
        ...
        <div class="bg-primary-50 border border-primary-200 rounded-2xl p-5">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-primary-800 mb-1">Informasi Penting</p>
                    <p class="text-xs text-primary-700">
                        Pastikan data yang diperbarui sudah benar. Data absensi akan digunakan untuk pelaporan kehadiran
                        guru.
                    </p>
                </div>
            </div>
