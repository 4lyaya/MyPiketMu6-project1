...
@section('content')
    ...
    <form action="{{ route('admin.absences.update', $absence) }}" method="POST" class="space-y-4 bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="reason" class="w-full border px-3 py-2 rounded">
                ...
                <option value="sakit" {{ old('reason', $absence->reason) == 'sakit' ? 'selected' : '' }}>Sakit</option>
                <option value="alpha" {{ old('reason', $absence->reason) == 'alpha' ? 'selected' : '' }}>Alpha</option>
                <option value="izin" {{ old('reason', $absence->reason) == 'izin' ? 'selected' : '' }}>Izin</option>
                <option value="terlambat" {{ old('reason', $absence->reason) == 'terlambat' ? 'selected' : '' }}>Terlambat
                </option>
                <option value="tugas_sekolah" {{ old('reason', $absence->reason) == 'tugas_sekolah' ? 'selected' : '' }}>
                    Tugas Sekolah</option>
            </select>
            @error('reason')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
